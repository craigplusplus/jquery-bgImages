 ...
 * @option {string} top The amount to shift eligable images down, w/ units. Defaults to 0px (no shift).
 *
 * @name $.bgImages
	...
	$.fn.bgImages = function( o ) {
				...
				for( var image_i = 0; image_i < images.length; image_i++ ) {
					...
					if( image !== '' ) {
						...
						if( typeof(opts.remove) !== 'string' || !image_is_tagged(image, opts.remove) ) {
							...
							debug( !!opts.shift_tagged 
								? 'We want to shift tagged images ('+opts.shift_tagged+')' 
								: 'We want to shift untagged images ('+opts.shift_tagged+')' );
		...
		function image_is_tagged(image, tags) {
			...
			for( var t_index = 0; t_index < tags.length; t_index++ ) {
				...
				if( !!tag &&  image.length >= tag.length && image.substr(image.length - tag.length) === tag ) {
